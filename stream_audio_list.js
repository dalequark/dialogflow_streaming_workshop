...
function getTimeRemaining() {
    ...
    if (!alarmStartTime || !alarmDurationMs) {
        return null;
    }
    return (currentTime - alarmStartTime - alarmDurationMs) / 1000;
}
...
async function handleResponse(dfStream, audio, queryResult) {
    ...
    else if (intent == "timeLeft") {
        let timeRemainingS = getTimeRemaining();
        if(!timeRemainingS) {
            audio = await dfStream.tts(`You have no timer set`);
        }
        else {
            audio = await dfStream.tts(`You have ${timeRemainingS} seconds left on the timer`);
        }
